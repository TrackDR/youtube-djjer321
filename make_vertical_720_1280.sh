...
END{
  for(i=0;i<n;i++){
    ...
    if(mode[i]==1){
      printf(",scale=720:-2:flags=lanczos,pad=720:1280:(ow-iw)/2:(oh-ih)/2,setsar=1,format=yuv420p[v%d];", i)
    } else {
      printf(",split=2[v%da][v%db];", i, i)
      printf("[v%da]%s,scale=720:-2:flags=lanczos[top%d];", i, top, i)
      printf("[v%db]%s,scale=720:-2:flags=lanczos[bot%d];", i, bot, i)
      printf("[top%d][bot%d]vstack=2,pad=720:1280:(ow-iw)/2:(oh-ih)/2,setsar=1,format=yuv420p[v%d];", i, i, i)
    }
  ...
  # >>> interleave video+audio per segment (fixes your error) <<<
